...
// --*/
 ...
 void test()
 {
    ...
    char heiheihei[maxN][maxN][maxN][10],hehehe[maxN][maxN][maxN][10];
    for (int i = 0;i < maxN;i++)
        for (int j = 0;j < maxN;j++)
            for (int k = 0;k < maxN;k++)
            {
                sprintf(heiheihei[i][j][k], "c%d%d%d", i, j, k);
                sprintf(hehehe[i][j][k],"d%d%d%d",i,j,k);
            }
    expr *x[maxN][maxN][maxN];
    expr *y[maxN][maxN][maxN];
    expr hehe = a || b;
    printf("233!\n");
    for (int i = 0;i < maxN;i++)
        for (int j = 0;j < maxN;j++)
            ...
            {
                printf("%s\n",heiheihei[i][j][k]);
                expr t = c.bool_const(heiheihei[i][j][k]);
                x[i][j][k] = &t;
                expr q = c.bool_const(hehehe[i][j][k]);
                y[i][j][k] = &q;
                hehe = hehe || x[i][j][k] || y[i][j][k];
            }
    solver s(c);
    s.add(hehe);
    //std::cout<<s<<std::endl;
    ...
    switch(s.check())
    {
        case unsat: std::cout<<"unsat"<<std::endl; break;
        case sat: std::cout<<"sat"<<std::endl; break;
...
void test1()
{
    ...
    expr conjecture = ! (x == 1) && (x == y);
    expr temp1 = c.bool_const("z");
    expr temp2 = temp1 && (x > y);
    solver s(c);
    s.add(conjecture && temp2);
    std::cout<<s<<"\n";
    try{
    model m = s.get_model();
